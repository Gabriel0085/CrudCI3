...
class Login extends CI_Controller {
    ...
    public function index(){
		...
		if($this->session->userdata("user_id")){
			...
			$this->template->show("restrita.php", $data);
		}
		else{
			$data = array(
				"scripts" => array(
	...
	public function ajax_save_course(){
		...
		if(empty($data["course_duration"])){
			...
			if(!($data["course_duration"] > 0 && $data["course_duration"] < 100)){
				$json["error_list"]["#course_duration"] = "Duração do curso deve ser maior que 0 horas e menor que 100 horas";
			}
		}
		...
		if(empty($data["user_email"])){
			...
			if($this->users_model->is_duplicated("user_email", $data["user_email"], $data["user_id"])){
				...
				if($data["user_email"] != $data["user_email_confirm"]){
					...
					$json["error_list"]["#user_email_confirm"] = "Os e-mails não são iguais";
				}
			}
		}
	...
	public function ajax_list_course(){
		...
		foreach($courses as $courses){
			...
			if($course->course_img){
				$row[] = '<img src="'.base_url().$course->course_img.'" style="mas-height: 100px;
				max-width: 100px">';
			}
			else{
				$row[] = "";
